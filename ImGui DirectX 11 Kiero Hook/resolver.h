...
namespace NaOrganization
{
	...
	namespace MidTerm
	{
		...
		namespace VmGeneralType
		{
			...
			void* Method::GetInvokeAddress() const
			{
				...
				if (mono_compile_method.IsValid())
					fn_ptr = mono_compile_method(method);

				auto invalid_ptr = false;

				if (!fn_ptr && !method)
				{

					std::string txt("failed to get method ptr / ");
					txt += GetName();

					MessageBoxA(NULL, txt.c_str(), NULL, NULL);
		...
		NaResolver::Exception::Exception(Level level, std::string message, ...) : level(level)
		{
			...
			//this->message = buf;
			//delete[] buf;
			MessageBoxA(NULL, message.c_str(), NULL, NULL);
		}
