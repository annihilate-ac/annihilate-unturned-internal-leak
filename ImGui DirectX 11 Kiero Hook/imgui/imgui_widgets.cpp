...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    ImRect container_bb(pos, pos + ImVec2(ImGui::GetContentRegionAvail().x, 30.f));
    ImRect button_bb(pos + ImVec2(29.f, 5.f), window->DC.CursorPos + ImVec2(159.f, 25.f));
    ...
    window->DrawList->AddRectFilled(container_bb.Min, container_bb.Max, IM_COL32(25, 20, 25, 255));

    window->DrawList->AddRectFilledMultiColor(container_bb.Min, { container_bb.Max.x, container_bb.Min.y + 6.f }, IM_COL32(95, 92, 103, 200), IM_COL32(95, 92, 103, 200), IM_COL32(27, 22, 27, 0), IM_COL32(27, 22, 27, 0));
    window->DrawList->AddRectFilledMultiColor(container_bb.Max, { container_bb.Min.x, container_bb.Max.y - 6.f }, IM_COL32(95, 92, 103, 200), IM_COL32(95, 92, 103, 200), IM_COL32(27, 22, 27, 0), IM_COL32(27, 22, 27, 0));

    window->DrawList->AddRectFilledMultiColor(container_bb.Min, { container_bb.Min.x + 6.f, container_bb.Max.y }, IM_COL32(95, 92, 103, 200), IM_COL32(27, 22, 27, 0), IM_COL32(27, 22, 27, 0), IM_COL32(95, 92, 103, 200));
    window->DrawList->AddRectFilledMultiColor(container_bb.Max, { container_bb.Max.x - 6.f, container_bb.Min.y }, IM_COL32(95, 92, 103, 200), IM_COL32(27, 22, 27, 0), IM_COL32(27, 22, 27, 0), IM_COL32(95, 92, 103, 200));

    window->DrawList->AddRect(container_bb.Min, container_bb.Max, ImGui::ColorConvertFloat4ToU32(ImGui::GetStyleColorVec4(ImGuiCol_AnimatedBorder)));

    window->DrawList->AddRectFilled(button_bb.Min, button_bb.Max, IM_COL32(25, 20, 25, 255));
    ...
    window->DrawList->AddRect(button_bb.Min, button_bb.Max, ImGui::ColorConvertFloat4ToU32(border));

    if (g.LogEnabled)
        LogSetNextTextDecoration("[", "]");
    RenderTextClipped(button_bb.Min + style.FramePadding, button_bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    float bb_frame_width = bb_frame.GetWidth();
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags, float thickness)
{
    ...
    else if (flags & ImGuiSeparatorFlags_Horizontal)
    {
        ...
        float x2 = window->Pos.x + (ImGui::GetContentRegionAvail().x - 10.f);
...
void ImGui::SeparatorTextEx(ImGuiID id, const char* label, const char* label_end, float extra_w)
{
    ...
    const ImRect bb(pos, ImVec2(window->WorkRect.Max.x - 10.f, pos.y + min_size.y));
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    ImRect container_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(ImGui::GetWindowWidth() - 10.f, 40.f));
    ImRect combo_bb(window->DC.CursorPos + ImVec2(29.f, 16.f), window->DC.CursorPos + ImVec2(159.f, 35.f));

    //const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(189.f, label_size.y + style.FramePadding.y * 2.0f));
    ItemSize(container_bb, style.FramePadding.y);
    if (!ItemAdd(container_bb, id))
        return false;
    ...
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ...
        if (value_x2 + arrow_size - style.FramePadding.x <= combo_bb.Max.x)
            RenderArrow(window->DrawList, ImVec2(value_x2 + style.FramePadding.y, combo_bb.Min.y + style.FramePadding.y), text_col, ImGuiDir_Down, 1.0f);
    }
    ...
    window->DrawList->AddRectFilled(combo_bb.Min, combo_bb.Max, IM_COL32(25, 20, 25, 255));
    window->DrawList->AddRect(combo_bb.Min, combo_bb.Max, ImGui::ColorConvertFloat4ToU32(ImGui::GetStyleColorVec4(ImGuiCol_Border)));
    ...
    if (label_size.x > 0)
        RenderText(combo_bb.Min + ImVec2(0.f, -(2.f + label_size.y)), label);
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    flags |= ImGuiColorEditFlags_NoSidePreview | ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoOptions;
    ...
    else if (flags & ImGuiColorEditFlags_PickerHueBar)
    {
        ...
        sv_cursor_pos.x = ImClamp(IM_ROUND(picker_pos.x + ImSaturate(S)     * sv_picker_size), picker_pos.x + 7.f, picker_pos.x + sv_picker_size - 14.f); // Sneakily prevent the circle to stick out too much
        sv_cursor_pos.y = ImClamp(IM_ROUND(picker_pos.y + ImSaturate(1 - V) * sv_picker_size), picker_pos.y + 7.f, picker_pos.y + sv_picker_size - 14.f);

        // Render Hue Bar
        for (int i = 0; i < 6; ++i)
            draw_list->AddRectFilledMultiColor(ImVec2(bar0_pos_x, picker_pos.y + i * (sv_picker_size / 6)), ImVec2(bar0_pos_x + bars_width, picker_pos.y + (i + 1) * (sv_picker_size / 6)), col_hues[i], col_hues[i], col_hues[i + 1], col_hues[i + 1]);
        float bar0_line_y = ImClamp(IM_ROUND(picker_pos.y + H * sv_picker_size), IM_ROUND(picker_pos.y + H * sv_picker_size) - 5.f, IM_ROUND((picker_pos.y) + H * sv_picker_size) + 10.f);
        RenderFrameBorder(ImVec2(bar0_pos_x, picker_pos.y), ImVec2(bar0_pos_x + bars_width, picker_pos.y + sv_picker_size), 0.0f);
        //RenderArrowsForVerticalBar(draw_list, ImVec2(bar0_pos_x - 1, bar0_line_y), ImVec2(bars_triangles_half_sz + 1, bars_triangles_half_sz), bars_width + 2.0f, style.Alpha);
        GetForegroundDrawList()->AddRectFilled(ImVec2(bar0_pos_x, bar0_line_y + 5.f), ImVec2(bar0_pos_x + bars_width, bar0_line_y), IM_COL32(133, 121, 145, static_cast<int>(g.Style.Alpha * 200.f)));
        GetForegroundDrawList()->AddRect(ImVec2(bar0_pos_x, bar0_line_y), ImVec2(bar0_pos_x + bars_width, bar0_line_y + 5), GetColorU32(ImGui::GetStyleColorVec4(ImGuiCol_Border)));
    }
    
    // Render cursor/preview circle (clamp S/V within 0..1 range because floating points colors may lead HSV values to be out of range)
    float sv_cursor_rad = 6.f;
    ...
    draw_list->AddRect(sv_cursor_pos, sv_cursor_pos + ImVec2{ sv_cursor_rad, sv_cursor_rad }, GetColorU32(ImGui::GetStyleColorVec4(ImGuiCol_Border)));
    draw_list->AddRect(sv_cursor_pos + ImVec2{ -1.f, -1.f }, sv_cursor_pos + ImVec2{ sv_cursor_rad + 1.f, sv_cursor_rad + 1.f }, IM_COL32(133, 121, 145, 255 * imgui_alpha));
    ...
    if (alpha_bar)
    {
        ...
        GetForegroundDrawList()->AddRectFilled(ImVec2(bar1_pos_x, bar1_line_y), ImVec2(bar1_pos_x + bars_width, bar1_line_y + 5), IM_COL32(133, 121, 145, static_cast<int>(g.Style.Alpha * 200.f)));
        GetForegroundDrawList()->AddRect(ImVec2(bar1_pos_x, bar1_line_y), ImVec2(bar1_pos_x + bars_width, bar1_line_y + 5), GetColorU32(ImGui::GetStyleColorVec4(ImGuiCol_Border)));
    }
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(ImGuiCol_AnimatedBorder));

    ImVec4 col_source = col_rgb;

    if (col_source.w < 1.f)
        RenderColorRectWithAlphaCheckerboard(window->DrawList, bb.Min + ImVec2(2.f, 2.f), bb.Max - ImVec2(2.f, 2.f), GetColorU32(col_source), grid_step, ImVec2(off, off));

    window->DrawList->AddRectFilled(bb.Min + ImVec2(2.f, 2.f), bb.Max - ImVec2(2.f, 2.f), GetColorU32(col_source));
